...
public:
  void setup(const string& name, float x, float y, float w, float h) {
	this->name = name;
	this->x = x;
	this->y = y;
	this->w = w;
  ...
  void draw(ofxTrueTypeFont* font) {
	...
	if (isEnabled)
	  ...
	  ofSetColor(100);
	font->drawString(name, x, y, TEXT_ALIGN_CENTER_MIDDLE);
  }
...
class CalculatorApp : public BaseApp
{
  ...
  void draw()
  {
	int x = 32;
	ofSetColor(220);
	bigFont.drawString(n1.size() > 0 ? n1 : "0", width - x, screenHeight - 16, TEXT_ALIGN_RIGHT_BOTTOM);
	if (op != 0) 
	  smallFont.drawString(n2 + opratorString(), width - x, screenHeight - 96, TEXT_ALIGN_RIGHT_BOTTOM);
